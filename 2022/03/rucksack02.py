def resolve(raw_data):
    parsed_data = list(filter(None, raw_data.split("\n")))
    scores = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ"
    points = []

    for index in range(int(len(parsed_data)/3)):
        index = index * 3
        first = parsed_data[index]
        second = parsed_data[index+1]
        third = parsed_data[index+2]

        badge = None
        for item in first:
            if item in second and item in third:
                badge = item
        points.append(scores.index(badge)+1)
    print(sum(points))
